...
import { X, Sparkles, User, Lock, Mail, Phone, CheckCircle2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-2xl relative">
        ...
        <div className="text-center mb-5">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-500 flex items-center justify-center text-white mx-auto mb-3 shadow-md">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Access your AI tools, Cloud storage, and subscriptions securely
          </p>
        </div>
        ...
        <div className="mb-4">
          <button
            ...
            className="w-full py-2.5 px-4 border border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold text-xs rounded-xl transition-all flex items-center justify-center gap-2 shadow-sm"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
            ...
            {isRegister && password.length > 0 && (
              <div className="mt-2 space-y-1">
                <div className="flex items-center justify-between text-[10px] font-bold">
                  ...
                  <span className={pwdScore >= 4 ? 'text-emerald-500' : pwdScore >= 2 ? 'text-amber-500' : 'text-rose-500'}>
          ...
          <button
            type="submit"
            className="w-full py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-md transition-all mt-2"
          >
            {isRegister ? 'Create Free Account' : 'Sign In to Account'}
          </button>
        </form>

        {/* Toggle Mode & Quick Sign Ins */}
